...
learning_rate=0.1
buckets=[(20, 20)] # our input and response words can be up to 10 characters long
PAD=[0] # fill words shorter than 10 characters with 'padding' zeroes
batch_size=10 # for parallel training (later)

input_data    = [[ord(c) for c in "hello"] + PAD * 15] * batch_size
target_data   = [[ord(c) for c in "world"] + PAD * 15] * batch_size
target_weights= [[1.0]*20] * batch_size # mask padding. todo: redundant --
...
def test():
    ...
    word = decode(words[0])
    print("step: " + str(step) + " perplexity: " + str(perplexity) + "output: hello " + str(words[0]))
    if word == "world":
...
with tf.Session() as session:
    model= BabySeq2Seq(vocab_size, target_vocab_size, buckets, size=20, num_layers=1, batch_size=batch_size)
